 ...
 * de la UI), sin necesidad de entrar por consola.
 *
 * <p>El backup automático de las 3 AM lo maneja cron dentro del contenedor de
 * backup. Este endpoint solo agrega el "correr ahora": reenvía la orden al
 * mini-servidor de disparo de ese contenedor ({@code backup:3002/run}), que
 * ejecuta el MISMO {@code backup.sh}.</p>
 *
 * <p>Cadena de seguridad: el browser llega acá por el gateway con JWT de ADMIN
 * (la ruta es {@code hasRole("ADMIN")} en el SecurityConfig); ms-auth llama al
 * contenedor de backup con una key interna que solo conoce el backend — nunca
...
public class BackupController {
    ...
    public BackupController(AuditoriaService auditoriaService,
                            @Value("${BACKUP_URI:http://backup:3002}") String backupUri,
