package com.gys.ms_catalogo.config;
 ...
 *
 * Estos IDs son referenciados en el DevDataInitializer de ms-pedidos.
 */
@Component
@Profile("dev")
public class DevDataInitializer implements CommandLineRunner {
    ...
    public DevDataInitializer(TipoTrabajoRepository repository) {
    ...
    public void run(String... args) {
        if (repository.count() > 0) {
            log.info("[GYS-DEV] Catálogo ya tiene datos — se omite la carga inicial.");
            return;
        }
        ...
        log.info("[GYS-DEV] {} tipos de trabajo cargados en el catálogo.", trabajos.size());
    }
}
