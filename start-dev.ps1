...
# Con -WithDocker: ademas arranca MySQL/RabbitMQ/MinIO via docker compose.
#
# Requisitos:
#   - Java 17+, Maven 3.8+
...
#   .\start-dev.ps1 -WithDocker    # idem + Docker (MySQL/Rabbit/Minio)
...
# -- 1. (Opcional) Docker ------------------------------------------
if ($WithDocker) {
    Write-Host "Paso 0: Docker (MySQL + RabbitMQ + MinIO)" -ForegroundColor Cyan
    Push-Location $root
    docker compose up -d
    Pop-Location
...
Write-Host ""
Write-Host "Paso 3: 5 ms de negocio en paralelo" -ForegroundColor Cyan
...
Start-Service-Window "5.produccion" "$root\ms-produccion" "mvn spring-boot:run"
Start-Service-Window "6.finanzas"   "$root\ms-finanzas"   "mvn spring-boot:run"
Start-Service-Window "7.stock"      "$root\ms-stock"      "mvn spring-boot:run"
...
Start-Service-Window "8.gateway" "$root\api-gateway" "mvn spring-boot:run"
Start-Sleep -Seconds 10
...
if (-not $NoFrontend) {
    ...
    Start-Service-Window "9.frontend" "$root\frontend-app" "Write-Host 'Recorda cambiar useMocks:false en environment.ts si queres usar el backend real' -ForegroundColor Yellow; ng serve --open"
}
...
Write-Host "    ms-produccion http://localhost:8084/h2-console (jdbc:h2:mem:gys_produccion)"
