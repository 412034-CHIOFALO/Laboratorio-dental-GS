...
public class DevDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (pedidoRepository.count() > 0) {
            log.info("[GS-DEV] ms-pedidos ya tiene datos — se omite la carga inicial.");
            return;
        }
        ...
        log.info("[GS-DEV] {} odontólogos cargados en ms-pedidos.", ods.size());
        ...
        log.info("[GS-DEV] {} pedidos de prueba cargados en ms-pedidos.", pedidos.size());
        log.info("[GS-DEV] Estados: RECIBIDO(1) EN_PROCESO(2) CONTROL(1) LISTO(1)");
    }
}
