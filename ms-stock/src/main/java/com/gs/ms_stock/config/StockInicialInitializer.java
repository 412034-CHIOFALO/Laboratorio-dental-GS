 ...
 * las categorías que ya usa el catálogo (coronas de zirconio/porcelana, prótesis
 * acrílicas/metálicas, ortodoncia removible, ATM). A diferencia de
 * {@link DevDataInitializer} (solo dev), este corre en TODOS los ambientes —
 * en producción el stock arranca vacío y sin esto no hay nada para descontar.
 *
 * <p>Idempotente por nombre, no por conteo de tabla: si el laboratorio ya cargó
 * materiales propios a mano, este seed no los duplica ni los pisa — solo agrega
 * los que todavía no existan.</p>
 *
 * <p>Las cantidades son un punto de partida razonable, no una receta real:
 * cuánto consume cada trabajo específico del catálogo lo define el laboratorio
 * desde Catálogo → editar tipo de trabajo → Receta, usando estos materiales.</p>
 */
@Component
